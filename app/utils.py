...
def handle_uploaded_file(cad):
    ...
    from_base64_to_picture.to_picture(str(data['MainPicture']).encode('utf-8'),
                                      os.path.join(settings.MEDIA_ROOT, 'imagenes/' + data['Name'] + '.jpg'))
    ...
    from_base64_to_picture.to_picture(str(data['OwnerPicture']).encode('utf-8'), os.path.join(settings.MEDIA_ROOT,
                                                                                              'imagenes/' + data[
                                                                                                  'Name'] + '_' + data[
                                                                                                  'OwnerName'] + '.jpg'))
    ...
    for item in gallery_set['Images']:
        count = count + 1
        imagen = Image()
        from_base64_to_picture.to_picture(str(item).encode('utf-8'), os.path.join(settings.MEDIA_ROOT,
                                                                                  'imagenes/' + gallery.title + '_' + str(
                                                                                      count) + '.jpg'))
