...
!function (context, $, undefined) {
    ...
    context.Carousel = new Class({
        ...
        initialize: function (options) {
            ...
            this.anim = new this.plugins[this.options.animation](this.elements, this.options, this).addEvents({
                change: function () {
                    this.fireEvent('change', Array.slice(arguments))
                }.bind(this), complete: function () {
                    this.running = false;
                    this.fireEvent('complete', Array.slice(arguments))
                }.bind(this)
            });
    ...
    context.Carousel.prototype.plugins.Move = new Class({
        ...
        reset: function () {
            ...
            //
            this.fx = new Fx.Elements(this.elements, this.options.fx).addEvents({
                complete: function () {
        ...
        move: function (current, direction) {
            ...
            this.fireEvent('change', [current, element]).fx.cancel().start(Object.map(this.elements, function (el, index) {
                if (!isNaN(index)) return up ? {top: el[property] - offset} : {left: el[property] - offset}
            }))
        }
    })
