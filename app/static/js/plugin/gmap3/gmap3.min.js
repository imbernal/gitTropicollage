...
!function (a, b) {
    ...
    function j(b, c, d, e, f) {
        ...
        var h = c.td || {}, i = {"id": e, "data": h.data, "tag": h.tag};
    ...
    function t(a, b, c, e, f) {
        var g = c ? r(e.td, !1, !0) : !1, h = g ? {"latLng": g} : e.td.address ? d(e.td.address) ? {"address": e.td.address} : e.td.address : !1, i = h ? R.get(h) : !1, j = this;
    ...
    function A() {
        function b(a) {
            return {"id": a.id, "name": a.name, "object": a.obj, "tag": a.tag, "data": a.data}
        }
        ...
        g.add = function (a, b, c, f) {
            ...
            var f, h;
            if (!d[a])return !1;
            if (b)if (c)for (f = d[a].length - 1; f >= 0 && (h = d[a][f], !b(e[h].tag)); f--); else for (f = 0; f < d[a].length && (h = d[a][f], !b(e[h].tag)); f++); else f = c ? d[a].length - 1 : 0;
        ...
        d(e) && (e = f(e)), j.run = function () {
            for (var d, f; d = g();) {
                if (O(b[d]))return h = d, f = a.extend(!0, {}, L[d] || {}, e[d].options || {}), void(d in k.latLng ? e[d].values ? u(e[d].values, b, b[d], {
                    ...
                    "opts": f,
                    "session": i
                }) : d in k.geoloc ? v(b, b[d], {"td": e[d], "opts": f, "session": i}) : b[d].apply(b, [{
                    "td": e[d],
                    "opts": f,
                    "session": i
                }]));
    ...
    function H(b, d, e) {
        function f(a) {
            H[a] || (delete I[a].options.map, H[a] = new L.classes.Marker(I[a].options), j(b, {"td": I[a]}, H[a], I[a].id))
        }
        ...
        function p() {
            if (!w && !y && z) {
                ...
                for (; ;) {
                    ...
                    if (g = [], A && !E) {
                        r = 10;
                        do for (n = g, g = [], r--, k = n.length ? h.getCenter() : I[v[b]].options.position, h = o(k), c = b; c < v.length; c++)B[c] || h.contains(I[v[c]].options.position) && g.push(c); while (n.length < g.length && g.length > 1 && r)
        ...
        g(), B.getById = function (a) {
            ...
            c = c || {}, c.id = h(c.id), B.clearById(c.id), c.options || (c.options = {}), c.options.position = a.getPosition(), j(b, {"td": c}, a, c.id), E[c.id] = H.length, F[H.length] = c.id, H.push(a), I.push(c), J.push(c.data || {}), x = !0
        }, B.td = function (a) {
            return I[a]
        }, B.value = function (a) {
            return J[a]
    ...
    function K(e) {
        ...
        function t(c) {
            ...
            return k.sort(function (a, b) {
                ...
                m ? (p = m.offset || [-m.width / 2, -m.height / 2], n = a.extend({}, h), n.options = a.extend({
                    ...
                    "offset": {"x": ("x" in p ? p.x : p[0]) || 0, "y": ("y" in p ? p.y : p[1]) || 0}
                }, h.options || {}), o = w.overlay({
                    "td": n,
                    "opts": n.options,
                    "latLng": r(f)
                }, !0), n.options.pane = "floatShadow", n.options.content = a(document.createElement("div")).width(m.width + "px").height(m.height + "px").css({"cursor": "pointer"}), q = w.overlay({
                    "td": n,
                    "opts": n.options,
                    "latLng": r(f)
                }, !0), h.data = {"latLng": r(f), "markers": []}, a.each(f.indexes, function (a, b) {
        ...
        function u(b, c, d) {
            ...
            return g || (b.td.values = [{"options": b.opts}]), b.td.values.length ? (q(), a.each(b.td.values, function (a, g) {
                var h, i, k, l, n = m(b, g);
                ...
                n.options.map = G, l = new M[c](n.options), f.push(l), h = y.add({"td": n}, c.toLowerCase(), l), j(e, {"td": n}, l, h)
        ...
        w._plan = function (a) {
            ...
            return f || (b.td.values = [{
                ...
                l.append(k.options.content), i = new J(G, k.options, r(k) || r(g), l), d.push(i), l = null, c || (h = y.add(b, "overlay", i), j(e, {"td": k}, i, h))
            }), c ? d[0] : void o(b, f ? d : d[0])) : void o(b, !1)
        }, w.marker = function (b) {
            var c, d, f, g = "values" in b.td, i = !G;
            return g || (b.opts.position = b.latLng || r(b.opts.position), b.td.values = [{"options": b.opts}]), b.td.values.length ? (i && q(), b.td.cluster && !G.getBounds() ? void M.event.addListenerOnce(G, "bounds_changed", function () {
                w.marker.apply(w, [b])
                ...
                var f, g, h = m(b, d);
                h.options.position = r(h.options.position ? h.options.position : d), h.options.position && (h.options.map = G, i && (G.setCenter(h.options.position), i = !1), g = new L.classes.Marker(h.options), c.push(g), f = y.add({"td": h}, "marker", g), j(e, {"td": h}, g, f))
            ...
            g || (c.latLng && (c.opts.position = c.latLng), c.td.values = [{"options": c.opts}]), a.each(c.td.values, function (a, h) {
                var i, k, l = m(c, h);
                l.options.position = r(l.options.position ? l.options.position : h.latLng), G || q(l.options.position), k = new L.classes.InfoWindow(l.options), k && (f(l.open) || l.open) && (g ? k.open(G, l.anchor || b) : k.open(G, l.anchor || (c.latLng ? b : c.session.marker ? c.session.marker : b))), d.push(k), i = y.add({"td": l}, "infowindow", k), j(e, {"td": l}, k, i)
            }), o(c, g ? d : d[0])
        }, w.circle = function (b) {
            var c = [], d = "values" in b.td;
            return d || (b.opts.center = b.latLng || r(b.opts.center), b.td.values = [{"options": b.opts}]), b.td.values.length ? (a.each(b.td.values, function (a, d) {
                var f, g, h = m(b, d);
                h.options.center = r(h.options.center ? h.options.center : d), G || q(h.options.center), h.options.map = G, g = new L.classes.Circle(h.options), c.push(g), f = y.add({"td": h}, "circle", g), j(e, {"td": h}, g, f)
            ...
            if (c.length)F().getElevationForLocations({"locations": c}, d); else {
                if (a.td.path && a.td.path.length)for (b = 0; b < a.td.path.length; b++)c.push(r(a.td.path[b]));
                c.length ? F().getElevationAlongPath({"path": c, "samples": a.td.samples}, d) : v.ack()
            }
            ...
            var c = [], d = "values" in b.td;
            return d || (b.td.values = [{"options": b.opts}]), b.td.values.length ? (a.each(b.td.values, function (a, d) {
                var f, g, h = m(b, d);
                h.options.bounds = s(h.options.bounds ? h.options.bounds : d), G || q(h.options.bounds.getCenter()), h.options.map = G, g = new L.classes.Rectangle(h.options), c.push(g), f = y.add({"td": h}, "rectangle", g), j(e, {"td": h}, g, f)
            }), void o(b, d ? c : c[0])) : void o(b, !1)
            ...
            return d || (b.td.values = [{"options": b.opts}]), b.td.values.length ? (a.each(b.td.values, function (a, d) {
                var f, g, h, k = m(b, d);
                G || q(), h = k.options, k.options.opts && (h = k.options.opts, k.options.url && (h.url = k.options.url)), h.map = G, g = i("3.10") ? new L.classes.KmlLayer(h) : new L.classes.KmlLayer(h.url, h), c.push(g), f = y.add({"td": k}, "kmllayer", g), j(e, {"td": k}, g, f)
            }), void o(b, d ? c : c[0])) : void o(b, !1)
        }, w.panel = function (b) {
            q();
